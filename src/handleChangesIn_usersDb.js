...
module.exports = async change => {
  ...
  nano
    .use('_users')
    .get(change.id, { revs: true, open_revs: 'all' }, function(error, body) {
      if (error) {
        return console.log(
      ...
      if (change.deleted) {
        ...
        nano
          .use('_users')
          .get(change.id, { rev: revOfOldDoc }, function(error, doc) {
            if (error) {
              if (error.statusCode === 404) {
        ...
        try {
          ...
          return console.log(
            'handleChangesIn_usersDb: error getting list of dbs:', error,
          )
        }
